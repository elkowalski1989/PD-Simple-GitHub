...
internal static class LaneAToolsChecks
{
    ...
    private static int CheckSidebarWiring()
    {
        ...
        Require(
            placeholders.Count == 8,
            $"Expected the 8 other-lane placeholders to remain, found {placeholders.Count}.");
    ...
    private static int CheckForwardingContract()
    {
        ...
        // Scope the forbidden-mechanism scan to the new Lane A code: the
        ...
        string forwardingBody = forwarding.Substring(
            forwarding.IndexOf("public void OpenSection", StringComparison.Ordinal));
        forwardingBody = forwardingBody.Substring(
            0, forwardingBody.IndexOf("public void AttachPresentation", StringComparison.Ordinal));
